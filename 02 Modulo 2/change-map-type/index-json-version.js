...
let mapStyles = null; // Se cargará desde el JSON
...
async function loadMapStyles() {
    try {
        ...
        mapStyles = await response.json();
        console.log('Estilos de mapa cargados exitosamente');
    } catch (error) {
        console.error('Error al cargar estilos de mapa:', error);
        // Fallback a estilos básicos si falla la carga
        mapStyles = {
...
function applyCustomMapType() {
    ...
    // Obtener estilo personalizado desde el archivo JSON
...
function getMapStyle(styleName) {
    return mapStyles && mapStyles[styleName] ? mapStyles[styleName] : null;
}
